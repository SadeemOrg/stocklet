...
class SiteController extends Controller
{
    ...
    public function videoDetail($slug, $id)
    {
        ...
        if (!$video->is_free && $user) {
            $downloads = Download::where('image_id', $video->id)->where('user_id', @$user->id)->where('premium', 1);
            $todayDownload = (clone $downloads)->whereDate('created_at', Carbon::now())->count();
            $monthlyDownload = (clone $downloads)->whereMonth('created_at', Carbon::now()->month)->count();
            $alreadyDownloaded = Download::where('image_id', $video->id)->where('user_id', @$user->id)->exists();
        }

        $relatedImages = Image::active()->where('id', '!=', $video->id)->where('category_id', $video->category_id)->with('user', 'likes')->orderBy('id', 'desc')->limit(8)->get();

        return view($this->activeTemplate . 'Video_details', compact('pageTitle', 'video', 'relatedImages', 'seoContents', 'todayDownload', 'monthlyDownload', 'alreadyDownloaded'));
    }
    ...
    public function videos($scope = null)
    {
        $pageTitle = 'videos';
        $images = Video::query();
    ...
    public function search(Request $request)
    {
        ...
        return view($this->activeTemplate . 'image_search', compact('pageTitle', 'images', 'collections','videos', 'imageCount', 'videoCount','collectionCount'));
    }
    ...
    {
        $videos = $this->searchvideo($request);
        $data['videoCount'] = (clone $videos)->count();
        if (!$onlyCount) {
            $data['videos'] = $videos->paginate(getPaginate(25));
        }
        return $data;
    }
    private function searchvideo($request)
    {


        $images = Video::active()->whereHas('category', function ($query) {
            $query->where('status', Status::ENABLE);
        })->with('likes', 'user');
