...
class PaymentController extends Controller
{
    ...
    public function depositInsert(Request $request)
    {
        ...
        if ($request->plan_id) {
            ...
            if ($plan->$price != $amount) {
                $notify[] = ['error', 'يجب أن يكون المبلغ مساويًا للخطة ' . $request->period . ' price'];
                return back()->withNotify($notify);
            }

            $amount = $plan->$price;
            $limitNotification = 'يرجى اتباع حد الدفع';
        }
    ...
    public function manualDepositUpdate(Request $request)
    {
        ...
        if ($data->plan_id) {
            ...
            $message    = 'تم التقاط طلب الدفع الخاص بك';
