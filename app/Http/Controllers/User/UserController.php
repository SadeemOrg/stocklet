...
class UserController extends Controller
{
    ...
    public function create2fa(Request $request)
    {
        ...
        if ($response) {
            ...
            $user->ts = 1;
            $user->save();
            $notify[] = ['success', 'تم تنشيط مصادقة Google بنجاح'];
            return back()->withNotify($notify);
        } else {
            $notify[] = ['error', 'رمز التحقق الخاطئ'];
            return back()->withNotify($notify);
        }
    }
    ...
    public function disable2fa(Request $request)
    {
        ...
        if ($response) {
            ...
            $user->ts = 0;
            $user->save();
            $notify[] = ['success', 'تم إلغاء تنشيط اثنين من المصادقة بعامل بنجاح'];
        } else {
            $notify[] = ['error', 'رمز التحقق الخاطئ'];
        }
        return back()->withNotify($notify);
    ...
    {
        if (auth()->user()->kv == 2) {
            $notify[] = ['error', 'KYC الخاص بك قيد المراجعة'];
            return to_route('user.home')->withNotify($notify);
        }
        if (auth()->user()->kv == 1) {
            $notify[] = ['error', 'لقد تم التحقق من KYC بالفعل'];
            return to_route('user.home')->withNotify($notify);
        }
        $pageTitle = 'KYC Form';
    ...
    public function kycSubmit(Request $request)
    {
        ...
        $notify[] = ['success', 'تم إرسال بيانات KYC بنجاح'];
        return to_route('/')->withNotify($notify);
    }
    ...
    public function userDataSubmit(Request $request)
    {
        ...
        $notify[] = ['success', 'أكملت عملية التسجيل بنجاح'];
        return to_route('user.home')->withNotify($notify);
    }
    ...
    public function referralCommissionLog()
    {
        ...
        if (!$general->referral_system) {
            $notify[] = ['error', 'آسف ، نظام الإحالة غير متوفر حاليًا'];
            return back()->withNotify($notify);
        }
