    ...
    */
    ...
    public function register(Request $request)
    {
        ...
        if (preg_match("/[^a-z0-9_]/", trim($request->username))) {
            $notify[] = ['info', 'يمكن أن يحتوي اسم المستخدم على رسائل صغيرة وأرقام صغيرة فقط.'];
            $notify[] = ['error', 'لا توجد شخصية خاصة أو مساحة أو رسائل رأس المال في اسم المستخدم.'];
            return back()->withNotify($notify)->withInput($request->all());
        }

        if (!verifyCaptcha()) {
            $notify[] = ['error', 'قدمت Captcha غير صالحة'];
        ...
        if ($exist) {
            $notify[] = ['error', 'رقم الهاتف المحمول موجود بالفعل'];
