                            ...
                            @if ($pdata->secs != null)
                                @foreach (json_decode($pdata->secs) as $sec)
                                    <li class="highlight icon-move item">
                                        ...
                                        <span class="d-inline-block text-white  {{app()->getLocale() === 'ar' ? 'ms-auto me-2' : 'me-auto ms-2'}}"> {{ __(@$sections[$sec]['name']) }}</span>
                                        <i class=" {{app()->getLocale() === 'ar' ? 'me-auto' : 'ms-auto'}} d-inline-block remove-icon fa fa-times"></i>
                        ...
                        @foreach ($sections as $k => $secs)
                            @if (!@$secs['no_selection'])
                                <li class="highlight icon-move clearfix">
                                    ...
                                    <span class="d-inline-block text-white  {{app()->getLocale() === 'ar' ? 'ms-auto me-2' : 'me-auto ms-2'}}"> {{ __($secs['name']) }}</span>
                                    <i class="ms-auto d-inline-block remove-icon fa fa-times"></i>
                                    <input type="hidden" name="secs[]" value="{{ $k }}">
                                    @if ($secs['builder'])
                                        <div class="{{app()->getLocale() === 'ar' ? 'float-start' : 'float-end'}} d-inline-block manage-content">
...
@push('script-lib')
    <script src="{{ asset('assets/admin_reviewer/js/jquery-sortable.js') }}"></script>
@endpush
